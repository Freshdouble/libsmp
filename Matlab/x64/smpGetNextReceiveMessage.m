function [ message, success ] = smpGetNextReceiveMessage( )
...
if libisloaded('libsmp_x64')
    messageSize = calllib('libsmp_x64','libsmp_getNextReceivedMessageLength');
    if messageSize == 0
        message = zeros(1,1);
        success = false;
    else
        ...
        [~,cstr] = calllib('libsmp_x64','libsmp_getReceivedMessage',cstr);
        if cstr.size == 0
            message = zeros(1,1);
            success = false;
        else

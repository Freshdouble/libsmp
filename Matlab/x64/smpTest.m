...
smpInit(true); %Use library with Reed Solomon Codes change to false to use the library without reed solomon encoding


message = randi(255,20,1);
smpSendBytes(message); %Prepare message for sending
if smpSendMessagesCount() == 0 %Check how many messages are in the send buffer
    error('smp should have one message in the buffer at this point');
end
[sendMessage, success] = smpGetNextSendMessage(); %Get one message to send over the interface
if ~success
...
smpReceiveBytes(sendMessage); %Send the received bytes to the smp library

if smpMessagesToReceive() == 0 %Check if the smp library has received a valid message
    error('Data should be correct and therefore the receive counter should be greater than zero');
end

[receivedMessage, success] = smpGetNextReceiveMessage(); %Get received message from the buffer

...
public:
	...
	size_t Transmit(functorType &callback, const Iterator &start, const Iterator &end)
	{
		...
		if (length > InternalBufferLength || (length + crclength) > MaximumSupportedMessageLength) return 0;
	...
	std::tuple<size_t, const uint8_t*> GenerateSendPacket(const uint8_t* sourcedata, const size_t sourcelen, uint8_t* destinationData, size_t destinationLength)
	{
		uint8_t *messageStartPtr;
		auto sendlength = SMP_Send(sourcedata, sourcelen, destinationData, destinationLength, &messageStartPtr);
		return {sendlength, messageStartPtr};
	}

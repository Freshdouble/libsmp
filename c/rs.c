...
void
build_codeword (unsigned char msg[], int nbytes, unsigned char dst[], ecc_t* ecc)
{
  int i;
...
void
decode_data(unsigned char data[], int nbytes, ecc_t* ecc)
{
  int i, j, sum;
  for (j = 0; j < NPAR;  j++) {
    sum	= 0;
...
void
debug_check_syndrome (ecc_t* ecc)
{
  int i;
...
void
encode_data (unsigned char msg[], int nbytes, unsigned char dst[], ecc_t* ecc)
{

...
static void init_exp_table (ecc_t* ecc);


void
init_galois_tables (ecc_t* ecc)
{
...
init_exp_table (ecc_t* ecc)
{
  ...
  ecc->gexp[0] = 1;
  ecc->gexp[255] = ecc->gexp[0];
  ecc->glog[0] = 0;			/* shouldn't log[0] be an error? */
  ...
  for (i = 1; i < 256; i++) {
    ...
    p2 = p1;
    p1 = pinit;
    ecc->gexp[i] = p1 + p2*2 + p3*4 + p4*8 + p5*16 + p6*32 + p7*64 + p8*128;
    ecc->gexp[i+255] = ecc->gexp[i];
  }
...
{
  return (ecc->gexp[255-ecc->glog[elt]]);
}

...
void
Modified_Berlekamp_Massey (ecc_t* ecc)
{
  int n, L, L2, k, d, i;
  ...
  for (n = ecc->NErasures; n < NPAR; n++) {
    ...
    if (d != 0) {
      ...
      /* psi2 = psi - d*D */
      for (i = 0; i < MAXDEG; i++) psi2[i] = psi[i] ^ gmult(d, D[i], ecc);
	...
	for (i = 0; i < MAXDEG; i++) D[i] = gmult(psi[i], ginv(d, ecc), ecc);
	L = L2;
      }
  ...
  for(i = 0; i < MAXDEG; i++) ecc->Lambda[i] = psi[i];
...
/* given Psi (called Lambda in Modified_Berlekamp_Massey) and synBytes,
  ...
  */
void
compute_modified_omega (ecc_t* ecc)
{
  int i;
  ...
  zero_poly(ecc->Omega);
  for(i = 0; i < NPAR; i++) ecc->Omega[i] = product[i];

}

/* polynomial multiplication */
void
mult_polys (int dst[], int p1[], int p2[], ecc_t* ecc)
{
  int i, j;
  int tmp1[MAXDEG*2];
  ...
  for (i = 0; i < MAXDEG; i++) {
    ...
    /* scale tmp1 by p1[i] */
    for(j=0; j<MAXDEG; j++) tmp1[j]=gmult(p2[j], p1[i], ecc);
...
void
init_gamma (int gamma[], ecc_t* ecc)
{
  int e, tmp[MAXDEG];
...
void
compute_next_omega (int d, int A[], int dst[], int src[], ecc_t* ecc)
{
  int i;
  for ( i = 0; i < MAXDEG;  i++) {
...
int
compute_discrepancy (int lambda[], int S[], int L, int n, ecc_t* ecc)
{
  int i, sum=0;
...
{
  int i;
  for (i = 0; i < MAXDEG; i++) poly[i] = gmult(k, poly[i], ecc);
}
...
Find_Roots (ecc_t* ecc)
{
  ...
  for (r = 1; r < 256; r++) {
    ...
    if (sum == 0)
      {
	ecc->ErrorLocs[ecc->NErrors] = (255-r); ecc->NErrors++;
      }
  }
}
			 ...
			 ecc_t* ecc)
{
  ...
  for (i = 0; i < ecc->NErasures; i++) ecc->ErasureLocs[i] = erasures[i];
